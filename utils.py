...
def clean_up_mess(X_train, X_test, y_train, size_cutoff=1000, valid_size=0.2, random_state=0):
    ...
    X = X.merge(
        X.groupby('department').id_annonce.count().reset_index().rename(columns={'id_annonce': 'density_department'}),
        on='department',
        how='left'
    )
    ...
    X = X.merge(
        X.iloc[train_index].groupby(
            ...
            ).price_m2.mean().reset_index().rename(columns={'price_m2': 'price_m2_type_zipcode'}),
    ...
    X = X.merge(
        X.iloc[train_index].groupby(
            ...
            ).price_m2.mean().reset_index().rename(columns={'price_m2': 'price_m2_type_deptcode'}),
    ...
    X = X.merge(
        X.iloc[train_index].groupby(
            ['city', 'property_type']
            ).price_m2.mean().reset_index().rename(columns={'price_m2': 'price_m2_type_city'}),
    ...
    for quantile, quantile_name in zip([0.25, 0.5, 0.75], ['25', '50', '75']):
        X = X.merge(
            X.iloc[train_index].groupby(
                ['postal_code', 'property_type']
                ).price_m2.quantile(quantile).reset_index().rename(columns={'price_m2': f'price_m2_type_zipcode{quantile_name}'}),
            on=['postal_code', 'property_type'],
            how='left'
        )
        X = X.merge(
            X.iloc[train_index].groupby(
                ['department', 'property_type']
                ).price_m2.quantile(quantile).reset_index().rename(columns={'price_m2': f'price_m2_type_deptcode{quantile_name}'}),
            on=['department', 'property_type'],
            how='left'
        )
        X = X.merge(
            X.iloc[train_index].groupby(
                ['city', 'property_type']
                ).price_m2.quantile(quantile).reset_index().rename(columns={'price_m2': f'price_m2_type_city{quantile_name}'}),
            on=['city', 'property_type'],
            how='left'
        )

    X = X.merge(
            X.iloc[train_index].groupby(['postal_code', 'property_type']).price_m2.std().reset_index().rename(columns={'price_m2': f'price_m2_type_zipcode_std'}),
            on=['postal_code', 'property_type'],
            how='left'
    )
    X = X.merge(
        X.iloc[train_index].groupby(['department', 'property_type']).price_m2.std().reset_index().rename(columns={'price_m2': f'price_m2_type_deptcode_std'}),
        on=['department', 'property_type'],
        how='left'
    )
    X = X.merge(
        X.iloc[train_index].groupby(['city', 'property_type']).price_m2.std().reset_index().rename(columns={'price_m2': f'price_m2_type_city_std'}),
        on=['city', 'property_type'],
        how='left'
    )
